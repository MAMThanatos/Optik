...
if(isset($data->id) && isset($data->nama) && isset($data->password)) {
    ...
    if($mode === 'add') {
        ...
        $query = "INSERT INTO pengguna (nama_lengkap, username, password, role) VALUES ('$nama', '$username', '$password_hashed', '$role')";
        ...
        if (!empty($password_plain)) {
            $query = "UPDATE pengguna SET nama_lengkap = '$nama', password = '$password_hashed' WHERE username = '$original_id'";
        } else {
            $query = "UPDATE pengguna SET nama_lengkap = '$nama' WHERE username = '$original_id'";
        }
